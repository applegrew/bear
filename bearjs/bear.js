...
export class BearClient {
  ...
  _bindTerminal() {
    ...
    // xterm.js may also emit \x1b[A / \x1b[B via onData for the same
    // wheel event, so we set a short-lived flag to suppress the
    // arrow-key history navigation that would otherwise fire.
    let wheelGuard = false;
    this.term.element.addEventListener('wheel', (e) => {
      e.preventDefault();
      wheelGuard = true;
      setTimeout(() => { wheelGuard = false; }, 50);
      if (e.deltaY < 0) {
        this._scrollUp(3);
      } else if (e.deltaY > 0) {
        this._scrollDown(3);
    ...
    this.term.onData((data) => {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (ch === '\x1b' && data[i + 1] === '[') {
          ...
          // Suppress arrow keys generated by mouse wheel scroll
          if (wheelGuard && (arrow === 'A' || arrow === 'B')) continue;
